...
function kasutan_page_titre() {
	...
	$classe="entry-title";
	$titre=get_the_title();
	if(function_exists('get_field') && esc_attr(get_field('improutes_masquer_titre'))==='oui') {
		$masquer=true;
	}
	if(is_front_page(  )) {
		$masquer=true;
...
function kasutan_page_banniere($page_id=false,$use_defaut=false) {
	...
	if(!$use_defaut) {
		...
		$image_id=esc_attr(get_field('improutes_banniere_image',$page_id));
	}
	
	if(!$image_id || $use_defaut) {
		$image_id=esc_attr(get_field('improutes_bg_image','option'));//image par défaut
	}
...
function kasutan_actus_banniere() {
	...
	if(is_single()) {
		...
		$image_id=esc_attr(get_field('improutes_banniere_image'));
		if(!$image_id) {
